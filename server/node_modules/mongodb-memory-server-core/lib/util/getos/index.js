...
const log = debug_1.default('MongoMS:getos');
...
function getOS() {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        const osName = os_1.platform();
        // Linux is a special case.
        if (osName === 'linux') {
            return yield getLinuxInformation();
        }
        return { os: osName };
    });
}
...
function getLinuxInformation() {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        const upstreamLSB = yield utils_1.tryReleaseFile('/etc/upstream-release/lsb-release', parseLSB);
        if (!utils_1.isNullOrUndefined(upstreamLSB)) {
            log('getLinuxInformation: Using UpstreamLSB');
            return upstreamLSB;
        }
        const etcOsRelease = yield utils_1.tryReleaseFile('/etc/os-release', parseOS);
        if (!utils_1.isNullOrUndefined(etcOsRelease)) {
            log('getLinuxInformation: Using etcOsRelease');
            return etcOsRelease;
        }
        const usrOsRelease = yield utils_1.tryReleaseFile('/usr/lib/os-release', parseOS);
        if (!utils_1.isNullOrUndefined(usrOsRelease)) {
            log('getLinuxInformation: Using usrOsRelease');
            return usrOsRelease;
        }
        const etcLSBRelease = yield utils_1.tryReleaseFile('/etc/lsb-release', parseLSB);
        if (!utils_1.isNullOrUndefined(etcLSBRelease)) {
...
function parseOS(input) {
    ...
    return {
        ...
        id_like: (_f = input.match(OSRegex.id_like)) === null || _f === void 0 ? void 0 : _f[1].toLocaleLowerCase(),
    };
}
